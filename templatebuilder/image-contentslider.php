...
if ( !class_exists( 'avia_sc_image_content_slider' ) )
{
  class avia_sc_image_content_slider extends aviaShortcodeTemplate
  {
			/**
			...
			{
				$this->config['name']			= __('Image Content Slider', 'avia_framework' );
				$this->config['tab']			= __('Content Elements', 'avia_framework' );
				$this->config['icon']			= AviaBuilder::$path['imagesURL']."sc-contentslider.png";
				$this->config['order']			= 83;
				$this->config['target']			= 'avia-target-insert';
				$this->config['shortcode'] 		= 'av_image_content_slider';
				$this->config['shortcode_nested'] = array('av_image_content_slide');
			...
			function shortcode_handler($atts, $content = "", $shortcodename = "", $meta = "")
			{
				...
				$slider  = new avia_image_content_slider($atts);
				return $slider->html();
			}
...
if ( !class_exists( 'avia_image_content_slider' ) )
{
	class avia_image_content_slider
	{
		...
		public function html()
		{
            ...
            $output .= "<div {$data} class='avia-content-slider-element-container avia-content-slider-element-{$type} avia-content-slider avia-smallarrow-slider avia-content-{$type}-active avia-content-slider".avia_content_slider::$slider." avia-content-slider-{$total} {$class}' {$styling}>";
                ...
                foreach($content as $key => $value)
                {
                    ...
                    if($loop_counter == 1) $output .= "<div class='slide-entry-wrap' style='background-image: url({$bg_src}); background-position: {$position}; background-size: cover;'>";
